...
class Model:
    ...
    def delta(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        d = (forward - strike) / vol_std
        normal_delta = cp_sign * div_fac * ss.norm.cdf(cp_sign * d)
        return normal_delta
    ...
    def vega(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        vol_std = np.fmax(vol * np.sqrt(texp), 1.0e-16)
        d = (forward - strike) / vol_std
        normal_vega = disc_fac * np.sqrt(texp) * ss.norm.pdf(d)
        return normal_vega
    ...
    def gamma(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        vol_std = np.fmax(vol * np.sqrt(texp), 1.0e-16)
        d = (forward - strike) / vol_std
        normal_gamma = div_fac ** 2 / disc_fac * ss.norm.pdf(d) / vol_std
        return normal_gamma
